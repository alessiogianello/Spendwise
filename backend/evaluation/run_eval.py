...
def main() -> int:
    ...
    parser.add_argument("--model", help="Override CLAUDE_MODEL for this run (e.g. claude-sonnet-5)")
    ...
    if not settings.anthropic_api_key and not os.environ.get("ANTHROPIC_API_KEY"):
        print(
            "ANTHROPIC_API_KEY is not set. Set it in backend/.env or the environment "
    ...
    print_report(results, metrics, settings.claude_model)
